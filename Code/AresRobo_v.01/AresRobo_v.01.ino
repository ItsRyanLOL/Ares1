...
void loop() {
  if (verboseDebug) Serial.println("new loop!");
  ...
  if (int(analogValue) < minObstacleDistance) {
    ...
    if (desiredHeading != -1) {
      ...
      allStop();
      while (desiredHeading == -1) {
        updateDesiredHeading();
        if (verboseDebug) Serial.println("NO BEACON!");
        delay(10);
      }
    }
    matrix.writeDisplay();
  ...
  if (currentTime >= (lastCheckTime + interval)) {   //Check for obstacles if time since last check > interval
    ...
    if ((currentHeading <= addAngle(desiredHeading, -(headingThreashold)) && currentHeading >= addAngle(desiredHeading, headingThreashold)) && desiredHeading != -1) {
      // beaconTurn(desiredHeading);
      Serial.println("BEACON TURN!");
      return;
    }
...
void rightTurn(int heading) {
  ...
  motorGo(0, CCW, 1023);
  motorGo(1, CW, 1023);
  while (!(getHeading() > (addAngle(heading, -10)) && getHeading() < (addAngle(heading, 10)))) {
    Serial.println(desiredHeading);
